...
// allowed exec is `launchctl` (reading and restarting the daemon and updater
// jobs under launchd on macOS), and it must go through runCommand so tests
// can stub it. This test parses the package's non-test sources and fails on
// any other exec.
func TestNoExternalToolDependency(t *testing.T) {
	...
	// program named anywhere is launchctl (Updater.launchctl).
	...
	if n := strings.Count(src.String(), `u.command(`); n != 1 || !strings.Contains(src.String(), `u.command("launchctl"`) {
